...
encodable_unsigned!(u8: 8,
    ENCODE_U8, DECODE_U8, ENCODE_SIMD_U8, DECODE_SIMD_U8, encode_zz_shift_u8);

encodable_unsigned!(u16: 8,
    ENCODE_U16, DECODE_U16, ENCODE_SIMD_U16, DECODE_SIMD_U16, encode_zz_shift_u16);

encodable_unsigned!(u32: 8,
    ENCODE_U32, DECODE_U32, ENCODE_SIMD_U32, DECODE_SIMD_U32, encode_zz_shift_u32);

encodable_unsigned!(u64: 8,
    ENCODE_U64, DECODE_U64, ENCODE_SIMD_U64, DECODE_SIMD_U64, encode_zz_shift_u64);
        ...
        impl EncodePrivate<$it> for Unimodal<$it> {
            unsafe fn _encode(input: &[$it]) -> Result<Vec<u32>, super::Error> {
                ...
                for (a, &e_cnt) in e_counts.iter().enumerate() {
                    ...
                    // Should not fail since block contains at least one entry
                    let max_e_wd: u32 = bit_dist.iter().rposition(|x| *x != 0).unwrap() as u32;
                    ...
                    let wrd: u64 = (utility::words_for_bits(e_wd * e_cnt as u32)
                        + utility::words_for_bits(i_wd * x_cnt)
                ...
                *s_ptr = (n_blks as u32) << 2 | flag;
                ...
                for (a, &e_cnt) in e_counts.iter().enumerate() {
                    ...
                    // Write block header
                    *s_ptr =
                        i_wd << 29 | (x_cnt as u32) << 22 | x_wd << 15 | (e_cnt as u32) << 7 | e_wd;
                    s_ptr = s_ptr.offset(1);
